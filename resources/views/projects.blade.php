...
<body class="bg-[#f9fafb] dark:bg-dark-bg text-[#111827] dark:text-dark-text min-h-screen transition-colors duration-300 relative w-full selection:bg-indigo-500 selection:text-white">
    ...
    <div id="app-container" class="max-w-4xl mx-auto px-4 pt-8 pb-4 relative z-10 flex flex-col min-h-screen">
        ...
        <header class="mb-8 flex flex-col items-start">
            ...
            <h1 class="text-2xl md:text-3xl font-bold text-black dark:text-white display-font mt-3">All Projects</h1>
        </header>

        <!-- PROJECTS GRID -->
        <main class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            @foreach($projects as $project)
                <a href="{{ $project['demo'] }}" target="_blank" class="block rounded-xl p-5 space-y-3 bg-white/60 dark:bg-slate-900/40 subtle-border subtle-border-hover transition-all duration-300 group hover:shadow-sm">
                    <div class="space-y-1">
                        <h3 class="text-base font-bold text-slate-950 dark:text-white transition-colors">
                            {{ $project['title'] }}
                        </h3>
                    ...
                    <div class="flex flex-wrap gap-1.5 pt-1">
                        @foreach($project['technologies'] as $tech)
                            <span class="px-2 py-0.5 text-[11px] font-semibold rounded bg-slate-100 dark:bg-slate-800/80 subtle-border text-slate-700 dark:text-slate-300">
                                {{ $tech }}
                            </span>
                        @endforeach
                    </div>
